...
docker run --rm \
  ...
  -w /workspace \
  perl:5.40 \
  sh -c "cpanm --installdeps --notest . && prove -l t/"
```
...
services:
  tests:
    ...
    command: sh -c "cpanm --installdeps --notest . && prove -l t/"
```
...
Testcontainers for Perl reads `DOCKER_HOST` as the first step in [socket detection](../index.md).
